...
import za.co.wethinkcode.robots.command.FireCommand;
import za.co.wethinkcode.robots.command.HelpCommand;
import za.co.wethinkcode.robots.command.LaunchCommand;
import za.co.wethinkcode.robots.command.MoveCommand;
import za.co.wethinkcode.robots.command.ReloadCommand;
import za.co.wethinkcode.robots.command.RepairCommand;
import za.co.wethinkcode.robots.command.StateCommand;
import za.co.wethinkcode.robots.command.TurnCommand;
...
public class Server implements Runnable {
  ...
  /**
   * Sends a reload message to the client indicating that the reload command was
   * successful.
  ...
  /**
   * Sends a reload message to the client indicating that the robot has been
   * reloaded.
   * It includes the current state of the robot in the response.
   */
  ...
  private JsonObject handleCommand(final String commandName, final JsonArray args) {
    ...
    if (!commandName.equals("reload") && !commandName.equals("repair")
        && world.getCurrentRobot().getStatus() != OperationalStatus.DEAD) {
      assert currentRobot != null;
      if (currentRobot.getStatus() != OperationalStatus.DEAD)
        currentRobot.setStatus(OperationalStatus.NORMAL);
    }
