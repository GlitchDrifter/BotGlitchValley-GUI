...
public class Obstacle {
  ...
  /**
   * Checks if this obstacle blocks the path that goes from coordinate (x1, y1) to
   * (x2, y2).
   * Since our robot can only move in horizontal or vertical lines (no diagonals
   * yet), we can assume that either x1==x2 or y1==y2.
   *
    ...
    if (a.getX() == b.getX()) {
      return a.getX() >= this.topLeftX && a.getX() <= this.bottomRightX
          && ((a.getY() >= this.topLeftY && b.getY() <= this.bottomRightY)
              || (b.getY() >= this.topLeftY && a.getY() <= this.bottomRightY));
    } else if (a.getY() == b.getY()) {
      return a.getY() >= this.topLeftY && a.getY() <= this.bottomRightY
          && ((a.getX() >= this.topLeftX && b.getX() <= this.bottomRightX)
              || (b.getX() >= this.topLeftX && a.getX() <= this.bottomRightX));
    }
    return false;
  }
   ...
   * 
   * @param anotherObstacle - another Obstacle to be checked if it overlaps the
   *                        current
